...
|--------|------|---------|----------------|
| **Dataset Size** | Works well up to millions | Any size | Use HNSW for most applications |
...
### Use HNSW When:
- ✓ Most workloads (scales to millions of vectors with sufficient RAM)
...
**Scaling Up:**
```sql
-- Start with HNSW (default for most workloads)
CREATE INDEX docs_embedding_idx ON documents
USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- Consider IVFFlat only if HNSW index no longer fits in RAM
-- or write throughput is critical
...
- ✓ OpenAI text-embedding-3 models output normalized vectors, so inner product and cosine yield identical rankings
- ✓ Need slightly faster queries (skips normalization step)
