...
description: |
  ...
  **This project:** We use OpenAI text-embedding-3-small (1536) and store as vector(1536) in knowledge_chunks. halfvec is an optional future optimization; apply this skill's tuning (ef_search, iterative_scan, etc.) to our existing vector column.
---
...
- **Enable the extension** in each database: `CREATE EXTENSION IF NOT EXISTS vector WITH SCHEMA extensions;` (Supabase installs extensions in the `extensions` schema; set `search_path = public, extensions` in functions that use vector types)
