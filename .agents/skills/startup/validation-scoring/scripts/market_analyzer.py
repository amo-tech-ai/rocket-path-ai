...
def normalize_pipeline_format(data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Accept validator pipeline output format (validator_report.details.market_sizing)
    and normalize it to the standard market_data format.

    Pipeline format: {"tam": N, "sam": N, "som": N, "citations": [...]}
    Standard format: {"market_data": {"tam": N, "sam": N, "som": N, "growth_rate": N}}
    """
    normalized = dict(data)

    # Detect pipeline market_sizing format (has tam/sam/som at top level with citations)
    if 'tam' in data and 'sam' in data and 'som' in data and 'market_data' not in data:
        ...
        if 'growth_rate' in data:
            normalized['market_data']['growth_rate'] = data['growth_rate']
        if 'citations' in data:
            normalized['market_data']['citations'] = data['citations']

    # Detect nested pipeline format (report.details.market_sizing)
    if 'market_sizing' in data and 'market_data' not in normalized:
        ...
        if 'growth_rate' in ms:
            normalized['market_data']['growth_rate'] = ms['growth_rate']

    # Detect nested unit_economics from pipeline revenue_model
    if 'revenue_model' in data and 'business_data' not in normalized:
        ue = data['revenue_model'].get('unit_economics', {})
        if ue:
            normalized['business_data'] = {
                'cac': ue.get('cac', 0),
                'ltv': ue.get('ltv', 0),
            }
            if 'payback_months' in ue:
                normalized['business_data']['payback_period_months'] = ue['payback_months']
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("\nAccepts both standard format and validator pipeline output format:")
        print('  Standard: {"market_data": {"tam": N, "sam": N}, "business_data": {"cac": N, "ltv": N}}')
        print('  Pipeline: {"tam": N, "sam": N, "som": N, "citations": [...]}')
        print('  Pipeline: {"market_sizing": {"tam": N, ...}, "revenue_model": {"unit_economics": {...}}}')
        sys.exit(1)

    with open(sys.argv[1], 'r') as f:
        data = json.load(f)

    # Normalize pipeline format to standard format
    data = normalize_pipeline_format(data)
