...
function getSignal(score: number): 'go' | 'caution' | 'no-go' {
...
export function isV2Report(details: any): boolean {
...
export function ReportV2Layout({ report, companyName, startupMeta }: ReportV2LayoutProps) {
  ...
  return (
    <>
      ...
      <div className="max-w-[1000px] mx-auto px-4 lg:px-8 py-8 flex flex-col gap-8">
        {/* Luxury Hero — merged ExecutiveSummary + ReportHero */}
        {(() => {
          ...
          // Build VC-grade narrative: verdict → why it wins → what kills it → conclusion
          const parts: string[] = [];
          ...
          if (fatalItem) {
            riskParts.push(`The biggest risk: ${fatalItem.assumption || fatalItem}. If this turns out to be wrong, the entire business model falls apart.`);
          } else if (topComp) {
        ...
        {(d.risks_assumptions?.length > 0) && (
          <Reveal delay={100}>
            <SectionShell id="risk" number={5} title="Risks & Assumptions" agent="Scoring">
              {v2 && typeof d.risks_assumptions?.[0] === 'object' ? (
                ...
                <ul className="space-y-2">
                  {(d.risks_assumptions || []).map((r: string, i: number) => (
        ...
        <Reveal delay={100}>
          <SectionShell id="next-steps" number={7} title="Next Steps" agent="Composer">
            {v2 && typeof d.next_steps?.[0] === 'object' ? (
